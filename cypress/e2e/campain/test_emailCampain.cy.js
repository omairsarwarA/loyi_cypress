...
it('update customer data', () => {
    ...
    cy.get('.fas.fa-at').click();
    // cy.get('[href="https://sandbox.loyicard.com/en/campaigns/email/new"] > .fas').click({ force: true });
    cy.get('#select2-mlist_id-container').click({ force: true });
    cy.get('[aria-label="☰ Your Marketing Lists"] > .select2-results__options > .select2-results__option').click({ force: true });
    cy.get('#select2-message_template-container').click({ force: true });
    cy.get('li[aria-label="☰ Smart Templates"] > ul > li:first-child').first().click({ force: true });
    cy.get('#send_campaign_btn').click();
    cy.get('.btn-hero-warning').click();
